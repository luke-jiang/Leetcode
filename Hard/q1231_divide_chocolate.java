...
/** You have one chocolate bar that consists of some chunks. Each chunk has its own
  ...
  * Being generous, you will eat the piece with the minimum total sweetness and give
  * the other pieces to your friends.
  * Find the maximum total sweetness of the piece you can get by cutting the chocolate
  * bar optimally.
  *
  * Example 1:
  ...
  */
  ...
  class Solution {
    ...
    private boolean valid(int[] sweetness, int K, int mid) {
        int sum = 0;
        int chunks = 0;
        for (int s : sweetness) {
            sum += s;
