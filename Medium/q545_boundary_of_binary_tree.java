...
class Solution {
    ...
    private void traverse(TreeNode curr, boolean leftBound, boolean rightBound) {
        if (curr == null) {
            return;
        }
        if (leftBound) {
        ...
        traverse(curr.left, leftBound, rightBound && curr.right == null);
        traverse(curr.right, leftBound && curr.left == null, rightBound);
        if (rightBound) {
            res.add(curr.val);
        }
        if (!leftBound && !rightBound && curr.left == null && curr.right == null) {
            // leaf node case
            res.add(curr.val);
        }
    }
